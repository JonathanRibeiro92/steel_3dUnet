...
def process_slice(mesh, z, resolution):
    ...
    # Encontrar os limites dos vértices da fatia atual
    x_min_slice, y_min_slice = vertices_xy.min(axis=0)
    x_max_slice, y_max_slice = vertices_xy.max(axis=0)

    # Calcular as dimensões da fatia
    x_range = x_max_slice - x_min_slice
    y_range = y_max_slice - y_min_slice

    # Centralizar os vértices da fatia
    vertices_centered = vertices_xy - [x_min_slice, y_min_slice]
...
def generate_slices(mesh, num_slices, resolution=(256, 256)):
    ...
    with ThreadPoolExecutor() as executor:
        futures = [executor.submit(process_slice, mesh, z_min + i * slice_height, resolution) for i in range(num_slices)]
